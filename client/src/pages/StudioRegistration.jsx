...
const StudioReg = () => {
    ...
    const {setShowStudioReg, axios, getToken, setIsStudio} = useAppContext();
    ...
    const onSubmitHandler = async (event) => {
        try {
            ...
            const {data} = await axios.post(`/api/studios/`, {name, contact, address, city}, 
                {headers: {Authorization: `Bearer ${await getToken()}`}})

            if (data.success){
                toast.success(data.message)
                setIsStudio(true);
                setShowStudioReg(false);
            }
            else{
  ...
  return (
    <div onClick={() => setShowStudioReg(false)}
    className='fixed top-0 bottom-0 left-0 right-0 z-100 flex items-center justify-center bg-black/70'>
        <form onSubmit={onSubmitHandler} onClick={(e) =>e.stopPropagation()} className='flex bg-white rounded-xl max-w-4xl max-md:mx-2'>
            ...
            <div className='relative flex flex-col items-center md:w-1/2 p-8 md:p-10'>
                ...
                onClick={() => setShowStudioReg(false)}/>
                <p className='text-2xl font-semibold mt-6'>Register Your Studio</p>
                ...
                <div className='w-full mt-4'>
                    ...
                    id='name' type="text" placeholder='Type here' className='border border-gray-200 rounded w-full px-3 py-2.5 
                ...
                <div className='w-full mt-4'>
                    ...
                    id='contact' type="text" placeholder='Type here' className='border border-gray-200 rounded w-full px-3 py-2.5 
                    mt-1 outline-indigo-500 font-light'required/>
                </div>
                {/*Address */}
                <div className='w-full mt-4'>
                    ...
                    id='Address' type="text" placeholder='Type here' className='border border-gray-200 rounded w-full px-3 py-2.5 
                    mt-1 outline-indigo-500 font-light'required/>
                </div>
