...
return artist && (
  <div className='bg-black text-white py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
    
    {/*artist details*/}
    <div className='flex flex-col md:flex-row items-start md:items-center gap-2'>
      <h1 className='text-3xl md:text-4xl font-sans'>
        {artist.name} 
        <span className='font-sans text-sm text-gray-400'> {artist.studio.name} Studio</span>
      </h1>
    </div>
    ...
    <div className='flex items-center gap-1 mt-2'>
      <StarRating/>
      <p className='ml-2 text-gray-300'>200+ reviews</p>
    </div>
    ...
    <form 
      ...
      className='flex flex-col md:flex-row items-start md:items-center justify-between bg-gray-900
      shadow-[0px_0px_20px_rgba(255,255,255,0.1)] p-6 rounded-xl mx-auto mt-16 max-w-6xl'
    >
      <div className='flex flex-col flex-wrap md:flex-row items-start md:items-center gap-4 md:gap-10 text-gray-300'>
        <div className='flex flex-col'>
          <label htmlFor='date' className='font-medium text-white'>Date</label>
          <input 
            onChange={(e) => setDate(e.target.value)} 
            min={new Date().toISOString().split('T')[0]}
            type="date" id='date' required
            className='w-full rounded border border-gray-500 bg-black text-white px-3 py-2 mt-1.5 outline-none' 
          />
        </div>
        ...
        <div className='flex flex-col'>
          <label htmlFor='startTime' className='font-medium text-white'>Start Time</label>
          <input 
        ...
        <div className='flex flex-col'>
          <label htmlFor='endTime' className='font-medium text-white'>End Time</label>
          <input 
      ...
      <button 
        type='submit' 
        className='bg-purple-600 hover:bg-primary-dull active:scale-95 transition-all
